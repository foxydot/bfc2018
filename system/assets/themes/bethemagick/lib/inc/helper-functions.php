...
function msdlab_do_header() {
    add_filter('genesis_seo_title','msdlab_remove_link',10,3);
    genesis_markup( array(
        'html5'   => '<div %s>',
        'xhtml'   => '<div id="title-area">',
        'context' => 'title-area',
    ) );
    print '<a href="'.trailingslashit( home_url() ).'" class="site-title-link">';
    do_action( 'genesis_site_title' );
    do_action( 'genesis_site_description' );
    print '</a>';
    echo '</div>';
    remove_filter('genesis_seo_title','msdlab_remove_link',10,3);
}

function msdlab_remove_link($title, $inside, $wrap){
    $title = strip_tags($title, '<br>,<p>');
    return $title;
}
...
function msdlab_do_social_footer(){
    ...
    print '<div class="identity">'.get_bloginfo('name').', '.get_bloginfo('description').'</div>';
